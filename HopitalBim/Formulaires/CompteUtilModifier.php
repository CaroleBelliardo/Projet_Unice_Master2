...
<!-- MOOODIIIIIIFFF -->	
			...
			<legend> Adresse employé </legend> <!-- Titre du fieldset --> 
				...
				<input type="text" class="form-control" name="text_rue"  pattern="[A-Za-z]{1-100}" title="Caractère alphabétique, 100 caractères maximum" placeholder="<?php echo $utilisateurInfo['rue'] ;?>" value="<?php if(isset($error)){echo $text_rue;}else {echo $utilisateurInfo['rue'];}?>" /><br>

				<label for="text_ville">Ville </label>
				<input type="text" class="form-control" name="text_ville"  pattern="[A-Za-z]{1-150}" title="Caractère alphabétique, 150 caractères maximum" placeholder="<?php echo $utilisateurInfo['nomVilles'] ;?>" value="<?php if(isset($error)){echo $text_ville;}else {echo $utilisateurInfo['nomVilles'];}?>" /><br>
